...
#include "RW.h"
...
#if defined(RW__PC)
    #define RW_SetP4PinDriveMode(shift, mode)  \
    do { \
        RW_PC =   (RW_PC & \
                                (uint32)(~(uint32)(RW_DRIVE_MODE_IND_MASK << \
                                (RW_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (RW_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define RW_SetP4PinDriveMode(shift, mode)  \
        do { \
            RW_USBIO_CTRL_REG = (RW_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(RW_DRIVE_MODE_IND_MASK << \
                                    (RW_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (RW_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		RW_SetP4PinDriveMode(RW__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(RW_DR & (uint8)(~RW_MASK));
    drVal = (drVal | ((uint8)(value << RW_SHIFT) & RW_MASK));
    RW_DR = (uint32)drVal;
}
...
uint8 RW_Read(void)
{
    return (uint8)((RW_PS & RW_MASK) >> RW_SHIFT);
}
...
{
    return (uint8)((RW_DR & RW_MASK) >> RW_SHIFT);
}
...
void RW_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  RW_INTCFG & (uint32)(~(uint32)position);
    RW_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(RW_INTSTAT & RW_MASK);
	RW_INTSTAT = maskedStatus;
    return maskedStatus >> RW_SHIFT;
}
